...
import { fetchModels, indexDocuments, searchDocuments, ragAnswerStream, deleteCollection, fetchDocuments } from './api'
...
export default function App() {
  ...
  async function handleIndex() {
    if (files.length > 0) {
      try {
        const { files: indexed } = await fetchDocuments()
    ...
    try {
      ...
      setIndexStatus({ ok: true, message: `Indeksert ${res.indexed} tekstbiter.` })
      setDocRefresh(n => n + 1)
    } catch (e) {
  ...
  async function handleRag() {
    ...
    try {
      ...
      if (!points.length || searchQuery !== query) {
        ...
        points = res.points
        usedQuery = query
        currentFilteredCount = res.filtered_count
      ...
      <main className="max-w-3xl mx-auto px-4 py-8 space-y-8">
        ...
        <section className="space-y-3">
          ...
          <p className="text-sm text-gray-500">
            Last opp dokumentene du ønsker at AI-en skal søke i. Støttede formater: <strong>PDF, Word (.docx) og tekstfiler (.txt)</strong>.
            Du kan laste opp flere filer samtidig. Dokumentene lagres i databasen til du sletter dem manuelt.
          </p>
          ...
          <Collapsible title="Avanserte innstillinger">
            <div className="grid grid-cols-2 gap-4 text-sm">
              <label className="space-y-1">
                ...
                <input type="range" min={300} max={2000} value={chunkSize}
                  onChange={e => setChunkSize(+e.target.value)} className="w-full" />
                <p className="text-xs text-gray-400">Kortere biter gir mer presise treff, men mister kontekst. Lengre biter beholder mer sammenheng.</p>
              </label>
              <label className="space-y-1">
                <span className="text-gray-600">Overlapp: {overlap} tegn</span>
                <input type="range" min={0} max={400} value={overlap}
                  onChange={e => setOverlap(+e.target.value)} className="w-full" />
                <p className="text-xs text-gray-400">Sikrer at setninger ikke kuttes midt i en tanke ved grensen mellom to tekstbiter.</p>
              </label>
            </div>
          </Collapsible>
          ...
          {indexStatus && (
            <p className={`text-sm px-3 py-2 rounded-lg ${indexStatus.ok ? 'bg-green-50 text-green-700' : 'bg-red-50 text-red-700'}`}>
              {indexStatus.message}
            </p>
          )}
        ...
        <section className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <label className="space-y-1">
                ...
                <input type="range" min={1} max={10} value={k}
                  onChange={e => setK(+e.target.value)} className="w-full" />
                <p className="text-xs text-gray-400">Høyere verdi gir mer kontekst, men kan også introdusere mindre relevante biter.</p>
              </label>
              <label className="space-y-1">
                <span className="text-gray-600">Relevanterskel: {scoreThreshold.toFixed(2)}</span>
                <input type="range" min={0} max={1} step={0.05} value={scoreThreshold}
                  onChange={e => setScoreThreshold(+e.target.value)} className="w-full" />
                <p className="text-xs text-gray-400">Tekstbiter under terskelen filtreres bort. 1.0 = identisk, 0.0 = helt urelatert. Senk hvis du får for få treff.</p>
              </label>
            </div>
          </Collapsible>
          ...
          <div className="flex gap-3">
            ...
            <button
              onClick={handleRag}
              disabled={ragLoading || searching || !query.trim()}
        ...
        {/* Search results (only shown after explicit search, not after RAG) */}
        ...
        {streamingText && (
          <div className="bg-white border border-blue-200 rounded-lg overflow-hidden">
            ...
            <div className="px-4 py-3 text-sm text-gray-800 leading-relaxed whitespace-pre-wrap">
              {streamingText}<span className="inline-block w-0.5 h-4 bg-blue-500 animate-pulse ml-0.5 align-text-bottom" />
            </div>
          </div>
        )}
